...
| Count secret findings | `search_secrets` | `detector_name`, `sort_by` (`event_time`/`detector_name`), `limit` (read `total_count`; no `offset` — rows beyond `limit` are unreachable, label as a sample) |
